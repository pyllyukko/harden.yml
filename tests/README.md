...
* Certain hardenings like `kernel` & `network` are not tested. Also the following:
    * `apparmor`
    * `audit`
    * `entropy`
...
* Certain tasks that install additional software are only tested in Debian derivatives:
    * `clamav`
    * `aide`
    * `rkhunter`
    * `lynis`
...
| 10     | `login` | `password`    | `root`          | Password change with weak password (`changeme1`)       | `PAM_SUCCESS`   |
...
| 10      | `login` | `password`    | `root`          | Password change with weak password. Rejected by `pam_pwquality`.   | `PAM_AUTHTOK_ERR` |
